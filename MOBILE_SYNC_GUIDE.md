...
기본적으로 **공개 실시간 브로커(MQTT)** 로 자동 연결됩니다. 가입도, 설정값 입력도 필요 없어요.
- 맥북·핸드폰에서 **같은 주소**(https://kimbiseooososo.vercel.app/work/dashboard.html)를 열면
- 헤더 배지가 **🟢 동기화됨** 으로 바뀌고, 한쪽에서 체크하면 다른 쪽이 깜빡이며 자동 반영됩니다.

> ⚠️ 단, 이건 **무료 공개 브로커**라 가끔 느리거나 끊길 수 있는 "best-effort" 방식입니다.
